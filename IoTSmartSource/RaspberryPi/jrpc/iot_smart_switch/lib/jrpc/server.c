...
cJSON *discover(jrpc_context_t *ctx, cJSON *params, cJSON *id) {
 ...
 //BUTTON1
	  ...
	  cJSON_AddStringToObject(sensor, "id", BUTTON1);
	  cJSON_AddStringToObject(sensor, "type", "onoff");
	  cJSON_AddStringToObject(sensor, "name", "button1");
	  cJSON_AddTrueToObject(sensor, "notification");
	  cJSON_AddItemToArray(sensors, sensor);
	  
  //BUTTON2
	  sensor = cJSON_CreateObject();
	  cJSON_AddStringToObject(sensor, "id", BUTTON2);
	  cJSON_AddStringToObject(sensor, "type", "onoff");
	  cJSON_AddStringToObject(sensor, "name", "button2");
	  cJSON_AddTrueToObject(sensor, "notification");
	  cJSON_AddItemToArray(sensors, sensor);
	// ------ Event Discover ------
		...
		case 2:
			if(BUTTON2_C == 16){
				if(event_send_data.button2 != event_temp_data.button2){	
					if(event_send_data.button2 == 1 || event_send_data.button2 == 3){
