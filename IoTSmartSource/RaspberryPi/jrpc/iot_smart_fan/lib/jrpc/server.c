...
#define DEVICE_ID "b827ebaa6469" //Raspberry PI MAC Address
...
#define LAMP DEVICE_ID "-lamp-1"
#define FAN	DEVICE_ID "-fan-0"
...
cJSON *discover(jrpc_context_t *ctx, cJSON *params, cJSON *id) {
  ...
  //LAMP
	  ...
	  cJSON_AddStringToObject(sensor, "id", FAN);
	  cJSON_AddStringToObject(sensor, "type", "powerSwitch");
	  cJSON_AddStringToObject(sensor, "name", "fan0");
...
cJSON *sensor_set(jrpc_context_t *ctx, cJSON *params, cJSON *id) {
  ...
  char *sensorIdStr = NULL,*sensorIdStr2 = NULL;
	...
	if (sensorIdStr != NULL) { //very simple simulation
		...
		}else if(strcmp(sensorIdStr,LAMP)==0){
			if(!strcmp(sensorIdStr2,"on")){
				result = cJSON_CreateString("on");
				cJSON_AddStringToObject(result, "error", "NULL");
				act_recv_data.lamp = 1;
			}else if(!strcmp(sensorIdStr2,"off")){
				result = cJSON_CreateString("off");
				cJSON_AddStringToObject(result, "error", "NULL");
				act_recv_data.lamp = 0;
			}
		}else if(strcmp(sensorIdStr,FAN)==0){
			if(!strcmp(sensorIdStr2,"on")){
				result = cJSON_CreateString("on");
				cJSON_AddStringToObject(result, "error", "NULL");
				act_recv_data.fan = 1;
...
int act_recv(int sensor){
	switch(sensor){
		...
		break;
		case LAMP_C:
			return act_recv_data.lamp;
		break;
		case FAN_C:
			return act_recv_data.fan;
		break;
	}
